...
import {
  ...
  assertLastCapturedScenarioIs,
  assertSummaryFieldIs,
  assertLastQuestionTextIs,
  assertFilteredScenariosLengthIs,
  assertAdvanceCandidatesLengthIs,
  assertExportContains,
  assertRunDiffHasNewlyFailing,
  assertRunDiffHasNewlyPassing,
  assertRunDiffIsNull,
  assertDomainVocabularyIsNull,
  assertAdapterExistsForDomain,
  assertProjectContextIsNull,
  assertDomainStructureSuggestedNameIs,
  assertAdapterStructureDomainIs,
  assertAdapterStructureFirstActionIs,
  assertAdapterStructureIsNull,
  assertFirstFailureIs,
  assertTestTraceIs,
  assertTestTraceIsNull,
...
export const averMcpIntegrationAdapter = implement(averMcp, {
  ...
  assertions: {
    ...
    lastCapturedScenarioIs: async (session, input) => assertLastCapturedScenarioIs(session, input),
    summaryFieldIs: async (session, input) => assertSummaryFieldIs(session, input),
    lastQuestionTextIs: async (session, input) => assertLastQuestionTextIs(session, input),
    filteredScenariosLengthIs: async (session, input) => assertFilteredScenariosLengthIs(session, input),
    advanceCandidatesLengthIs: async (session, input) => assertAdvanceCandidatesLengthIs(session, input),
    exportContains: async (session, input) => assertExportContains(session, input),
    runDiffHasNewlyFailing: async (session, input) => assertRunDiffHasNewlyFailing(session, input),
    runDiffHasNewlyPassing: async (session, input) => assertRunDiffHasNewlyPassing(session, input),
    runDiffIsNull: async (session) => assertRunDiffIsNull(session),
    domainVocabularyIsNull: async (session, input) => assertDomainVocabularyIsNull(session, input),
    adapterExistsForDomain: async (session, input) => assertAdapterExistsForDomain(session, input),
    projectContextIsNull: async (session) => assertProjectContextIsNull(session),
    domainStructureSuggestedNameIs: async (session, input) => assertDomainStructureSuggestedNameIs(session, input),
    adapterStructureDomainIs: async (session, input) => assertAdapterStructureDomainIs(session, input),
    adapterStructureFirstActionIs: async (session, input) => assertAdapterStructureFirstActionIs(session, input),
    adapterStructureIsNull: async (session, input) => assertAdapterStructureIsNull(session, input),
    firstFailureIs: async (session, input) => assertFirstFailureIs(session, input),
    testTraceIs: async (session, input) => assertTestTraceIs(session, input),
    testTraceIsNull: async (session, input) => assertTestTraceIsNull(session, input),
  },
})
