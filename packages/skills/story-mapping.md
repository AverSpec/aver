...
For each confirmed behavior, run `packages/agent-plugin/scripts/&lt;backend&gt;/scenario-capture.sh --title "..." --body "..."` with:
...
Link each scenario to the backlog item using `packages/agent-plugin/scripts/&lt;backend&gt;/backlog-update.sh` with the scenario issue numbers.
